...
class MomentumStrategy(BaseStrategy):
    ...
    def get_sell_signals(self, portfolio, all_data):
        """Implements the sell logic using configurable parameters."""
        signals = []
        for ticker, position in portfolio.items():
            ...
            # Rule 2: Price crosses below EMA 21 (trailing stop)
            if not sell_signal:
    ...
    def get_buy_signals(self, stock_list, portfolio, all_data, max_positions):
        ...
        for ticker in stock_list:
            ...
            if ticker in portfolio or ticker not in all_data or len(all_data[ticker]) < 2:
                continue
            ...
            # FIX: Ensure price is above the EMAs and they are aligned for a stronger trend signal
            is_trend_aligned = (
                latest_data.get('Close', 0) > latest_data.get('EMA_11', 0) and
                latest_data.get('EMA_11', 0) > latest_data.get('EMA_21', 0)
            )
            ...
            if is_trend_aligned and is_strong_momentum and is_volume_spike:
                signals.append({
                    'ticker': ticker,
                    'price': latest_data['Close'],
